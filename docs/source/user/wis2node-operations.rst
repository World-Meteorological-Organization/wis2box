...
For more information see the section `WIS2 Node Registration`_ on the WMO community pages.
...
and any other WMO regulations specific to the data type.

It is recommended to regularly check the Grafana dashboards inside wis2box to verify that data is being published and no errors are occurring.

Maintaining your discovery metadata
-----------------------------------

To verify that your metadata records have been cached by the Global Discover Catalogue you can search for your centre-id in any of the WIS2 Global Discovery Catalogue.

For example to see the records for centre-id `br-inmet` you can query the Global Discovery Catalogues as follows:
...
If any of your datasets are missing, please login to wis2box-management container and republish your datasets using the following command:
...
Please make sure to use wis2box version 1.1 or later to ensure the metadata is validated before republishing.

If the Global Discovery Catalogue contain outdated metadata records, you can use the following command to send a notification to delete the record:
...
Make sure to replace `urn:wmo:md:my-centre-id:my-local-id` with the actual identifier of your metadata record as recorded in the Global Discovery Catalogue.
